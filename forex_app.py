...
if app_mode == "Automated FX Fund Simulator":
    st.header("⚙️ Institutional Manual Execution Terminal (Persistent Storage)")
    ...
    FX_DB_FILE = "fx_portfolio.json"

    def load_fx_db():
        if os.path.exists(FX_DB_FILE):
            try:
                with open(FX_DB_FILE, "r") as f:
                    return json.load(f)
            except:
                pass
    ...
    def save_fx_db(data):
        with open(FX_DB_FILE, "w") as f:
            json.dump(data, f, indent=4)

    # Load data from physical disk file into current session
    st.session_state.fx_account = load_fx_db()
    ...
    if st.sidebar.button("Wipe Sandbox & Reset Cash"):
        if os.path.exists(FX_DB_FILE):
            os.remove(FX_DB_FILE)
        st.session_state.fx_account = {"cash": 50000.00, "positions": {}, "ledger": []}
        save_fx_db(st.session_state.fx_account)
        st.rerun()
            ...
            if available_buys:
                ...
                if st.button(f"🚀 Execute Market BUY Order: {selected_buy}"):
                    if st.session_state.fx_account["cash"] >= trade_size:
                        ...
                        save_fx_db(st.session_state.fx_account) # Save to Disk File
                        st.toast(f"Locked {selected_buy} into portfolio database!")
                        st.rerun()
                    else:
